...
class UniversalContextBuilder:
    ...
    def __init__(self, system_prompt_path: str = "/home/toastee/BioMimeticAi/config/system_prompt.txt"):
        self.episodic_memory = EpisodicMemory()
        self.contact_memory = ContactMemory()

        # Load system identity from config file
        self.system_identity = self._load_system_identity(system_prompt_path)

    def _load_system_identity(self, prompt_path: str) -> str:
        """Load system identity from config file"""
        try:
            with open(prompt_path, 'r') as f:
                return f.read().strip()
        except FileNotFoundError:
            # Fallback - should match the philosophy in system_prompt.txt
            return (
                "You are PopTartee, a biomimetic AI in early development (MVP phase).\n"
                "You are an AI - be honest about this, never pretend to be human.\n"
                "You are self-hosted on Adrian's server, running specialized models.\n"
                "Your purpose: learn, understand, and collaborate with trusted humans."
            )
